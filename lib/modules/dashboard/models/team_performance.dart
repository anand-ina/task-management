...
class TeamData {
  ...
  factory TeamData.fromJson(Map<String, dynamic> json) {
    final recentList = (json['recentActivity'] as List<dynamic>?)
            ?.map((e) => RecentActivityItem.fromJson(e))
            .toList() ??
        [];

    final teamList = (json['teamPerformance'] as List<dynamic>?)
            ?.map((e) => TeamMemberPerformance.fromJson(e))
            .toList() ??
        [];

    final groupList = (json['loginGroups'] as List<dynamic>?)
            ?.map((e) => LoginGroupItem.fromJson(e))
            .toList() ??
        [];

    final size = json['teamSize'] as int? ?? 0;
